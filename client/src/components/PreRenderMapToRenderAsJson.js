    ...
    for (var i = 0; i < p.length; i++) {
      if (p[i].list.length > h) {
        ...
        for (var iter = 0; iter < gp.length; iter++) {
          ...
          if (obj.education) {
            result = gp[iter].details.find(groupObj => {
          ...
          if (result != undefined) {
            var insert=[]
            var index = gp[iter].details.indexOf(result)
            ...
            gp[iter].details[index].nextItem = nextIt
            gp[iter].details[index].length = (parseInt(gp[iter].details[index].length) + parseInt(obj.length))
            gp[iter].details[index].numOfEntries = gp[iter].details[index].numOfEntries + 1
            break;
          }
        }
        ...
        if (result === undefined) {
          ...
          gp[h].details = gp[h].details.concat(insert)
        }
      }
    }
  }
